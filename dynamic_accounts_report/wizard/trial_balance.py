...
class TrialView(models.TransientModel):
    ...
    def get_filter_data(self, option):
        ...
        journals = r.journal_ids if r.journal_ids else self.env['account.journal'].search(
            company_domain)
    ...
    def get_dynamic_xlsx_report(self, data, response, report_data, dfr_data):
        ...
        if filters.get('date_from'):
            sheet.merge_range('A4:B4', 'From: ' +
                              filters.get('date_from'), date_head)
        if filters.get('date_to'):
            sheet.merge_range('C4:D4', 'To: ' +
                              filters.get('date_to'), date_head)
        ...
        for rec_data in report_data_main:
            ...
            if filters.get('date_from'):
                if rec_data.get('Init_balance'):
                    sheet.write(
                        row, col + 2, rec_data['Init_balance']['debit'], txt)
                    sheet.write(
                        row, col + 3, rec_data['Init_balance']['credit'], txt)
                else:
